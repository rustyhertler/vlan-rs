...
5. **Config & CLI** ✅ *done*. TOML topology file (`--config <path.toml>`), live reconfig (`SIGHUP` reloads — full teardown/rebuild, not a diff), per-port/VLAN counters (`SIGUSR1` dumps to stderr). `scripts/config-reload-smoke-test.sh` passed: real TAP ports torn down and rebuilt under `SIGHUP` without restarting the process.

All five planned phases are done as of this line.

Stretch: scripted netns test harness in CI ✅ and `cargo-fuzz` on the parser ✅ — see `docs/stretch-goals.md`. Unscheduled: MAC aging, QinQ, loop guard, small web dashboard.
...
| 5 — config & CLI | `scripts/config-reload-smoke-test.sh` passed — a TOML file reproduces a port/VLAN layout on startup; `SIGHUP` tears down and rebuilds real TAP ports to match an edited config, without restarting the process (briefly interrupts every port, not just changed ones — see Control, above); `SIGUSR1` dumps counters |
