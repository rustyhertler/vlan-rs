...
`tests/dashboard.rs` itself needs neither `sudo` nor a real TAP device — unlike every other stretch/phase acceptance test — since `dashboard::serve` only ever talks to a channel, never a kernel interface; it's tested with a real `TcpListener`/`TcpStream` on `127.0.0.1:0` under the existing plain `cargo test` step, no CI change needed for that part. But the channel's *other* end (`daemon::run`'s `select!` arm calling `dashboard::render_counters_json(&switch)` against a `Switch` fed by real TAP traffic) was still only exercised manually until `scripts/dashboard-smoke-test.sh` — same two-namespace topology as `scripts/netns-smoke-test.sh`, plus a real `ping` and a `curl` against `/api/counters` asserting both ports show `frames_out > 0` — closed that gap and joined the `smoke-tests` CI job alongside the others.
