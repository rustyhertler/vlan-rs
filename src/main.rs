...
use vlan_rs::io::TapPort;
use vlan_rs::switch::{Forward, PortId, Switch, Vlan};

/// `<tap-name>:<vlan-id>` for each port, e.g. `tap0:10 tap1:10 tap2:20`.
...
fn parse_args(args: impl Iterator<Item = String>) -> io::Result<Vec<(String, Vlan)>> {
    args.map(|arg| {
...
async fn main() -> io::Result<()> {
    let specs = parse_args(std::env::args().skip(1))?;
    if specs.is_empty() {
